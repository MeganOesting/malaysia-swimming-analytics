...
def load_foreign_athletes():
    ...
    try:
        # Check new location first, fallback to old
        athlete_info_path = project_root / "data" / "athletes" / "AthleteINFO.csv"
        if not athlete_info_path.exists():
            # Old location (if data folder still exists at root)
            athlete_info_path = project_root.parent / "data" / "athletes" / "AthleteINFO.csv"
...
def load_club_mapping():
    ...
    try:
        # New location: reference_data/imports/
        clubs_path = project_root.parent / "reference_data" / "imports" / "Clubs_By_State.xlsx"
...
def main():
    ...
    # New location: meets/active/2024-25/ (fallback to old location)
    meets_dir = project_root.parent / "meets" / "active" / "2024-25"
