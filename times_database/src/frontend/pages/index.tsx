...
export default function Home() {
  ...
  const filteredResults = results.filter(result => {
    ...
    if (appliedMeets.length > 0) {
      // The frontend now uses actual meet IDs from the database
      // We need to find the meet ID that corresponds to this result's meet
      // The result has meet_name and meet_code, but we need to match against the meet ID
      // For now, let's use a simple approach: check if the meet name matches
      const meetName = result.meet;
      const meetCode = result.meet_code;
      
      // Map meet names to the actual meet IDs from the database (using first ID for each meet)
      const meetMapping = {
        'SUKMA 2024': ['00268b81-5428-4164-b221-51e2aa1de512'],
        'SEA Age 2025': ['76f7dae9-0fa7-4f2f-9561-66742d718c11'],
        'MO 2025': ['37c4b0fb-0fa2-40e1-9bea-c9f7f8aa20e2'],
        'MIAG 2025': ['972191a7-36dc-4cdc-8c95-76ea270a8c78'],
        'States 2025': ['87c63b18-eba0-4967-a878-8457d2d43adc']
      };
      
      const meetIds = meetMapping[meetName];
      if (!meetIds || !meetIds.some(id => appliedMeets.includes(id))) {
        return false;
      }
    }
    ...
    if (appliedEvents.length > 0) {
      ...
      const strokeLabel = strokeMap[result.stroke] || result.stroke;
      const eventString = `${result.distance}m ${strokeLabel}`;
