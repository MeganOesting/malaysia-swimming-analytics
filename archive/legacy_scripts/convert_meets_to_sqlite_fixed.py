...
def process_standard_meet_file(df, meet_info, foreign_athletes, club_mapping, course='LCM'):
    ...
    - Row 0 (index 0): Title row - IGNORE (e.g., "Malaysia, 16 Aug 2024 - 28 Aug 2024, Open")
    - Row 1 (index 1): Header row - Column names (COURSE, GENDER, DISTANCE, STROKE, FULLNAME, etc.)
    - Row 2+ (index 2+): Data rows - Actual swimming results
    ...
    # Excel file structure is fixed:
    # Row 0 = Title (ignore)
    # Row 1 = Headers (COURSE, GENDER, DISTANCE, STROKE, FULLNAME, etc.)
    # Row 2+ = Data rows
    header_row = 1  # Always row 1 (index 1) contains headers
    
    # Skip row 0 (title row) and row 1 (header row)
    # Data starts at row 2 (index 2)
    print(f"  File structure: Row 0=Title (ignored), Row 1=Headers, Row 2+=Data")
    print(f"  Processing data from row 2 (index 2) onwards")
    ...
    # Row 0 = Title (ignore), Row 1 = Headers (ignore), Row 2+ = Data
    for index in range(2, len(df)):
        try:
            # Get the row as a pandas Series
            row = df.iloc[index]
            
            # Skip if row is None or not a valid Series
            if row is None:
                continue
            if not hasattr(row, 'iloc') and not hasattr(row, '__getitem__'):
                continue
            
            # Get row length safely - pandas Series have length
            try:
                # For pandas Series, len() works directly
                row_len = len(row)
            except (TypeError, AttributeError) as e:
                # If len() fails, skip this row
                print(f"    Warning: Could not get length for row {index}: {e}")
                continue
            
            # Skip rows with insufficient columns (need at least 5 for basic data: COURSE, GENDER, DISTANCE, STROKE, FULLNAME)
            if row_len < 5:
                continue
            
            # Skip completely empty rows (first 2 columns are NaN)
            try:
            ...
            # Map columns using standard format with safe access
            # Note: course from column 0 might differ from detected course, use detected course
            course_from_data = str(row.iloc[0]).strip() if row_len > 0 and pd.notna(row.iloc[0]) else None
            # Use the detected course passed as parameter (more reliable)
            result_course = course
            gender = str(row.iloc[1]).strip().upper() if row_len > 1 and pd.notna(row.iloc[1]) else ''
            distance = int(row.iloc[2]) if row_len > 2 and pd.notna(row.iloc[2]) else None
            stroke = str(row.iloc[3]).strip().upper() if row_len > 3 and pd.notna(row.iloc[3]) else ''
            name = str(row.iloc[4]).strip() if row_len > 4 and pd.notna(row.iloc[4]) else ''
            birthdate = str(row.iloc[5]).strip() if row_len > 5 and pd.notna(row.iloc[5]) else ''
            nation = str(row.iloc[6]).strip().upper() if row_len > 6 and pd.notna(row.iloc[6]) else ''
            club_code = str(row.iloc[7]).strip() if row_len > 7 and pd.notna(row.iloc[7]) else None
            time_str = str(row.iloc[8]).strip() if row_len > 8 and pd.notna(row.iloc[8]) else ''
            time_seconds_numeric = float(row.iloc[9]) if row_len > 9 and pd.notna(row.iloc[9]) else None
            aqua_points = row.iloc[10] if row_len > 10 and pd.notna(row.iloc[10]) else None
            rudolph_points = float(row.iloc[11]) if row_len > 11 and pd.notna(row.iloc[11]) else None
            place = int(row.iloc[12]) if row_len > 12 and pd.notna(row.iloc[12]) else None
            result_meet_date = row.iloc[13] if row_len > 13 and pd.notna(row.iloc[13]) else None
                ...
                if isinstance(result_meet_date, (pd.Timestamp, datetime)):
                    ...
                    for fmt in date_formats:
                        try:
                            ...
                            continue
            meet_city = str(row.iloc[14]).strip() if row_len > 14 and pd.notna(row.iloc[14]) else None
            meet_name = str(row.iloc[15]).strip() if row_len > 15 and pd.notna(row.iloc[15]) else None
            club_name = str(row.iloc[16]).strip() if row_len > 16 and pd.notna(row.iloc[16]) else ''
    ...
    # Also check the header row (row 1, index 1) for the date
    if len(df.columns) > 13 and 1 < len(df):
        try:
            header_val = df.iloc[1, 13] if pd.notna(df.iloc[1, 13]) else None
        except (IndexError, TypeError):
            header_val = None
    ...
    # Also check data rows (starting from row 2, index 2)
    # Row 0 = Title (ignore), Row 1 = Headers (ignore), Row 2+ = Data
    for index in range(2, len(df)):
        try:
            row = df.iloc[index]
            ...
            # Check row length safely
            try:
                row_len = len(row)
            except (TypeError, AttributeError):
                continue
            
            # Need at least 14 columns to have result_meet_date (column 13, index 13)
            if row_len <= 13:
                continue
...
def process_seag_file(df, meet_info, foreign_athletes, club_mapping):
    ...
    for index in range(header_row + 1, len(df)):
        try:
            ...
            # Skip if row is None or not a valid Series
            if row is None or not hasattr(row, 'iloc'):
                continue
...
def process_meet_file(file_path, meet_info, foreign_athletes, club_mapping):
    ...
    try:
        ...
        # IMPORTANT: Read with header=None to preserve all rows
        # Structure: Row 0=Title (ignore), Row 1=Headers, Row 2+=Data
        ...
        # Check first few sheets for course indicator (look in row 2, index 2, column 0)
        course = 'LCM'  # Default to LCM
        for sheet_name in sheet_names[:3]:  # Check first 3 sheets
            try:
                # Read with header=None to get all rows including title and header rows
                df_sample = pd.read_excel(file_path, sheet_name=sheet_name, header=None, engine='xlrd' if file_path.suffix == '.xls' else None)
                # Check row 2 (index 2) column 0 for course indicator
                if len(df_sample) > 2:
                    try:
                        course_cell = str(df_sample.iloc[2, 0]).strip().upper() if pd.notna(df_sample.iloc[2, 0]) else ''
                        if 'SCM' in course_cell:
                            course = 'SCM'
                            break
                        elif 'LCM' in course_cell:
                            course = 'LCM'
                            break
                    except:
                        pass
        ...
        for sheet_name in valid_sheet_names:
            ...
            try:
                ...
                for i in range(min(5, len(df))):
                    print(f"        Row {i}: {df.iloc[i].tolist()[:5] if len(df) > i else 'N/A'}")
